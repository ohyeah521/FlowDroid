...
public class IFDSSolver extends AbstractIFDSSolver {
	...
	protected ConcurrentHashMultiMap<Pair<SootMethod, Abstraction>, PathEdge<Unit, Abstraction>> jumpFunctions = new ConcurrentHashMultiMap<>();

	@SynchronizedBy("thread safe data structure")
	protected volatile IGarbageCollector garbageCollector;
	...
	protected AbstrationDependencyGraph abstDependencyGraph;
	...
	protected IGarbageCollector createGarbageCollector() {
		...
		NormalGarbageCollector gc = new NormalGarbageCollector(icfg, jumpFunctions, endSummary, abstDependencyGraph);
		gc.setSleepTimeSeconds(sleepTime);
		logger.info("sleep time is {}", sleepTime);
		@SuppressWarnings("unchecked")
		GCSolverPeerGroup<Pair<SootMethod, Abstraction>> gcSolverGroup = (GCSolverPeerGroup<Pair<SootMethod, Abstraction>>) solverPeerGroup;
	...
	protected void scheduleEdgeProcessing(PathEdge<Unit, Abstraction> edge, Pair<SootMethod, Abstraction> orgSrc) {
		...
		Abstraction sourceVal = edge.factAtSource();
		Unit target = edge.getTarget();
		Abstraction targetVal = edge.factAtTarget();
		...
		if (isSelfLoopEdge) {
			...
			Pair<SootMethod, Abstraction> abst = new Pair<>(sm, edge.factAtSource());
			Map<EndSummary, EndSummary> map = new MyConcurrentHashMap<>();
			Map<EndSummary, EndSummary> sumMap = endSummary.putIfAbsentElseGet(abst, map);
	...
	protected boolean applyEndSummaryOnCall(final Abstraction d1, final Unit n, final Abstraction d2,
	...
	protected Set<IncomingRecord> incoming(Abstraction d1, SootMethod m) {
		Set<IncomingRecord> inc = incoming.get(new Pair<>(m, d1));
		return inc;
	}
