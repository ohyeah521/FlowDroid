...
public class IFDSSolver extends AbstractIFDSSolver {
	...
	protected ConcurrentHashMultiMap<SootMethod, PathEdge<Unit, Abstraction>> jumpFunctions = new ConcurrentHashMultiMap<>();
...
//		DefaultGarbageCollector gc = new DefaultGarbageCollector(icfg, jumpFunctions);
		ThreadedGarbageCollector gc = new ThreadedGarbageCollector(icfg, jumpFunctions);
	...
	protected void notifyEdgeScheduled(PathEdge<Unit, Abstraction> edge) {
		super.notifyEdgeScheduled(edge);
	...
	protected void scheduleEdgeProcessing(PathEdge<Unit, Abstraction> edge, ScheduleTarget scheduleTarget) {
		super.scheduleEdgeProcessing(edge, scheduleTarget);
		garbageCollector.gc();
	}
	...
	public Abstraction addFunction(PathEdge<Unit, Abstraction> edge) {
		PathEdge<Unit, Abstraction> oldEdge = jumpFunctions.putIfAbsent(icfg.getMethodOf(edge.getTarget()), edge);
